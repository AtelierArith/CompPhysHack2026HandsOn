...
- フリーランスソフトウェアエンジニア．
  - 大学教員と科学計算の共同開発を行っている．
- GitHub: [terasakisatoshi](https://github.com/terasakisatoshi)
- Organization: [AtelierArith](https://github.com/AtelierArith)
  - 自分が作成した Julia パッケージを公開している.

## Actually

これから紹介するソフトウェアは AI を活用しています．
私が担当した 9 割の実装は AI によって生成したものです．これは事実です．
私は Julia のような動的な言語が好きで，コンパイルする言語は苦手でした．
AI のおかげで Julia 以外のプログラミング言語を活用し多くのソフトウェアを高速に開発することができるようになりました．


## 最近行ってること: Organizing SparseIR ecosystem

- Hiroshi Shinaoka, Hitoshi Mori, Markus Wallerberger との共同開発
...
## 最近行ってること: `tensor4all` メンバーとの開発

- [tensor4all-rs](https://github.com/tensor4all/tensor4all-rs): A Rust implementation of tensor networks
  - Porting [Jutho/Strided.jl](https://github.com/Jutho/Strided.jl) to Rust [strided-rs](https://github.com/tensor4all/strided-rs)
...
Julia から Rust を呼び出すことができるパッケージ．Cxx.jl の設計を参考にした．
...
::: {.fragment}
ソースコードはどこですか？ (Git が使われていない．バージョン管理されてない)
:::
...
- 研究者がソフトウェアを開発する場合には再現性のあるソフトウェアを作ることが重要です．
...
これらは現代的なパッケージマネージメントシステムを備えており，他の人と共同で開発することを可能にします．
:::
...
- LLM は多くの知識を持っているが，研究者のように最先端の知識を持っているとは限らない．
  - 何を実装するか，しないかは人間の能力に左右される．
- ソフトウェアエンジニアはコーディングする時間から解放されるが，次のような能力は必要になる：
  - どのような技術をどの場面で使うか
  - 高品質なソフトウェアを作成・維持するためのアーキテクチャを考える
  - 意思決定をする上位の能力
- コーディング以外に研究者と議論するための最低限のドメイン知識も求められるようになる．

![](assets/scientist_and_engineer.png)
...
![](assets/ai_slack_off.png)

- ベストプラクティスな実装とは限らない．
  - Julia だと多重ディスパッチを使わない，`export` を多用する
  - Rust だと `.clone()` の多用をする．パフォーマンスの劣化
- 最低限のコードのビルド，実行方法，コーディングの知識，プログラミング言語の知識は必要．
...
- AI の導入によって下記のような局所的な議論から解放される:
  - 言語の学習コスト
  - とても細かい言語の機能・シンタックス
  - コーディングスタイル
- 下記のような大域的な議論に集中するべき
  - 「ユーザにとって体験が良いソフトウェアをどのように設計するか」
  - 状況・目的に応じてどのような技術，プログラミング言語を選択すべきか」
  -「高品質なソフトウェアをどのように維持するべきか」
  - 何を実装するか実装すべきでないか」
...
- ドメイン知識を身についけることが大事
  - 物理学者とコミュニケーションする程度の物理学の知識
- Rust で書かれたコードは Web Assembly の技術で Web で実行することができる．
  - 数値計算のデモを Web で公開・メンテナンスをする仕事は出てくるかも？
...
- 研究者は忙しくなる．
  - 今までできなかったけれど，やりたかったことができるようになる．
  - それに応じて仕事も増えてくるはず
- 研究者は暇にならない．
  - ある研究者は `Claude Code MAX $200` を契約し，AI をフル活用したせいで目眩を引き起こした．
  - 私は `$100` プランまでダウングレードすることを推奨した．ジョークのようで本当の話

![](assets/scientist_dizzy.png)
...
TODO [preparation.qmd](../handouts/preparation/preparation.qmd) に記述する
